"use strict"; //To make compiler understand, that treat this javascript code as a new standard version of JavaScript

alert(3+3); // This does not working with Node JS

/* 
    we can choose not to use Semicolon ';' 
    only use semicolon when, want to seperate two lines of code in a single line
    
    example - console.log("Bishal"); console.log("Biswas");
*/

/* 
    For JavaScript documentation, follow- MDN Documentation - https://developer.mozilla.org/en-US/docs/Web/JavaScript
    and https://tc39.es/
 */

let name= "bishal" //String
let age = 28 //integer and another is bigint
let letter = 'b' //character
let pi = 3.14 //float/Double
let isAlive = true //Boolean
let noValue; //undefined
let nothingStore = null //null defines empty value
//symbols => they are used in react